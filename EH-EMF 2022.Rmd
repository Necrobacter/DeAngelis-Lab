...
rowtovarname<- as.string(keggezwide[1,])
colnames(keggezwide) <- rowtovarname
keggezwide <- keggezwide[-c(1),]
keggezwide <- sapply(keggezwide, as.numeric)
...
<!-- Original output gave "diver", a shannons H for all enzymes and not samples. -->

```{r}
diver <- diversity(keggez[,-(1)])
diver2 <- diversity(keggezwide)
```
